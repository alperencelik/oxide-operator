...
import (
	...
	oxidecontroller "github.com/alperencelik/oxide-operator/internal/controller/oxide"
...
func init() {
	...
	utilruntime.Must(oxidev1alpha1.AddToScheme(scheme))
...
func main() {
	...
	// The external watcher polls Oxide for each ready Instance, feeds drift back into the
	// Instance controller and keeps .status.state fresh between reconciles.
	instanceWatcher := watcher.NewExternalWatcher(&oxideclient.InstanceFetcher{Client: mgr.GetClient()},
		watcher.WithDefaultPollInterval(pollInterval),
		watcher.WithLogger(ctrl.Log.WithName("external-watcher")),
		watcher.WithMetrics("Instance"),
		watcher.WithAutoRegister(mgr.GetCache(), &oxidev1alpha1.Instance{}, oxideclient.InstanceConfigExtractor,
			watcher.AutoRegisterWithFilter(watcher.EventFilter{
				Update: func(oldObj, newObj client.Object) bool { return oldObj.GetGeneration() != newObj.GetGeneration() },
			}),
			watcher.AutoRegisterWithReadinessRetry(watcher.ReadinessRetryConfig{InitialInterval: 20 * time.Second}),
		),
	)
